...
pub fn store_ibc_messages( deps: DepsMut,
    env: Env,
    info:MessageInfo,
    messages: Vec<Message>)->Result<Response,ContractError>{
        let config = CONFIG.load(deps.storage)?;
        if(config.gateway_contract!=info.sender){
               return Err(ContractError::Unauthorized);
        }
        messages.into_iter().map(|msg|{
            let key=msg.hash();
            IBC_MESSAGES.save(deps.storage, &key, &msg).unwrap();
        });
    Ok(Response::new())
...
mod test {
    ...
    fn mock_config(governance: Addr, voting_threshold: MajorityThreshold) -> Config {
        Config {
            ...
            gateway_contract: Addr::unchecked("gateway"),
        }
    }
