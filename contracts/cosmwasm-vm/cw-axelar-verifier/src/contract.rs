...
pub fn instantiate(
    ...
    let config = Config {
        ...
        gateway_contract:deps.api.addr_validate(&msg.gateway_address)?,
    };
...
pub fn execute(
    ...
    match msg {
        ...
        ExecuteMsg::SaveIbcMessages { messages }=>{
            execute::store_ibc_messages(deps, env, info,messages)
        }
    }
...
mod test {
    ...
    fn setup(workers: Vec<Worker>) -> OwnedDeps<MockStorage, MockApi, MockQuerier, Empty> {
        ...
        let config = Config {
            ...
            gateway_contract: Addr::unchecked("gateway_address")
        };
